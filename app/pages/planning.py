...
SIMULATIONS = 2_000
sizes = tuple(np.linspace(max(500, required // 4), required * 2, 10).astype(int))


@st.cache_data(show_spinner=False)
def run_power_simulation(sample_sizes: tuple[int, ...], baseline_cr: float, lift: float, seed: int):
    return power_curve(sample_sizes, baseline_cr, lift, simulations=SIMULATIONS, seed=seed)
...
fig.add_scatter(x=curve.sample_size * 2 / (monthly / 30), y=curve.simulated_power, mode="markers", name=f"{SIMULATIONS:,} simulated tests per point")
fig.update_layout(title="Theoretical and simulated power agree as evidence grows", xaxis_title="Test duration (days)", yaxis_title="Power", yaxis_tickformat=".0%")
