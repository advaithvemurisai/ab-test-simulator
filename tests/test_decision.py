...
def test_decision_prioritizes_validity_and_guardrails():
    invalid = decide(srm_p_value=0.0001, funded_lift=.3, funded_p_value=.001, revenue_ci_low=1, fraud_relative_lift=0, fraud_p_value=1)
    guardrail = decide(srm_p_value=.4, funded_lift=.3, funded_p_value=.001, revenue_ci_low=1, fraud_relative_lift=.8, fraud_p_value=.01)
    assert invalid.label == "DON'T TRUST"
    assert guardrail.label == "DON'T SHIP"


def test_decision_can_ship_or_keep_testing():
    ship = decide(srm_p_value=.4, funded_lift=.1, funded_p_value=.01, revenue_ci_low=0, fraud_relative_lift=.1, fraud_p_value=.4)
    keep_testing = decide(srm_p_value=.4, funded_lift=.02, funded_p_value=.4, revenue_ci_low=1, fraud_relative_lift=.1, fraud_p_value=.4)
